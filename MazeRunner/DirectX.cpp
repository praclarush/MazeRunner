...
**
** File: DirectX.cpp
** Description: Code for directX support code
**
** Author: Nathan Bremmer & Book Source Code
** -------------------------------------------------------------------------*/
...
LPDIRECT3DSURFACE9 mouseSurface = NULL;
...
bool DirectInput_Init(HWND hwnd)
{
	...
	//THIS STUFF WAS SUPPOSE TO DRAW A MOUSE ON THE SCREEN WHEN SHOWCURSOR IS SET TO FALSE; IT DOESN"T WORK,
	//AS SUCH NO MOUSE SUPPORT PAST MAIN SCREEN AS I CANT GET ANYTHING TO WORK IN REGARDS TO MOUSE OPERATIONS
	D3DXIMAGE_INFO imageInfo;

	D3DXGetImageInfoFromFile("pointer.bmp", &imageInfo);

	d3ddev->CreateOffscreenPlainSurface(imageInfo.Width, imageInfo.Height, imageInfo.Format, D3DPOOL_DEFAULT, &mouseSurface, NULL);

	D3DXLoadSurfaceFromFile(mouseSurface, NULL, NULL, "pointer.bmp", NULL, D3DX_FILTER_NONE, 0xFF000000, NULL);
	
	d3ddev->SetCursorProperties(0, 0, mouseSurface);
	d3ddev->SetCursorPosition(0, 0, D3DCURSOR_IMMEDIATE_UPDATE);
...
void DirectInput_Update()
{
	...
	mouse_location.x += mouse_state.lX;
	mouse_location.y += mouse_state.lY;
	mouse_location.z += mouse_state.lZ;

	d3ddev->SetCursorPosition(mouse_location.x, mouse_location.y, D3DCURSOR_IMMEDIATE_UPDATE);
...
CSound *LoadSound(string filename)
{
	...
	result = dsound->Create(&wave, s, DSBCAPS_CTRLVOLUME);
...
{
	long fixedVolume = (DSBVOLUME_MIN + (((DSBVOLUME_MAX - DSBVOLUME_MIN) * volume) / 100));

	sound->Play(0, 0, fixedVolume);
}
...
void LoopSoundVolume(CSound *sound, long volume)
{

	long fixedVolume = (DSBVOLUME_MIN + ((DSBVOLUME_MAX - DSBVOLUME_MIN) * volume) / 100);


	sound->Play(0, DSBPLAY_LOOPING, fixedVolume);
...
void SetSoundVolume(CSound *sound, long volume)
{
	long fixedVolume = (DSBVOLUME_MIN + ((DSBVOLUME_MAX - DSBVOLUME_MIN) * volume) / 100);
	sound->SetVolume(fixedVolume);
}
